...
Prima riga `$:.unshift(File.dirname(__FILE__)+"/../../../lib")` carica il path da cui prendere la libreria; attenzione però al fatto che il path cambia a seconda di dove si trova il file che stai creando rispetto alla libreria Rubyvis. Quindi `require 'rubyvis'` carica il file ruby necessario per poter usare la libreria. Sottolineo il fatto che entrambi i comandi sono necessari. Il metodo `areacharter` riceve in ingresso come dati una hash i quali elementi hanno come attributi le coordinate :x e :y e come secondo parametro il path(opzionale) in cui verrà salvato il file SVG di output. C'è un file `areacharter.rb` di test nella medesima directory.  
