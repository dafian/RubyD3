module Rubyvis
  ...
  class Mark
    ...
    # Create a new Mark
    def initialize(opts=Hash.new, &block)  # DDD qui crea la base dei children children
      @_properties=[]
      opts.each {|k,v|
      ...
      @scale=1
      @scene=nil
      @children= [] # BOO aggiunto io per poter fare annidamenti soprattutto per il group
      if block
        execute(&block)
    ...
    def sibling                               # IO l'ho modificata quasi tutta'
      if self.parent.type=='group' && !self.parent.scene[0].data.nil? #!(self.parent._data.value.is_a Proc)
        if self.parent._data.value.size > 1
          ...
          j=0
          while self.parent.children[j].type != self.type
            j=j+1
          end
          return self.parent.scene[i-1].children[j][0] if i>0
        end
      end
    ...
    def mark_bind() # :nodoc:
      if children.size >0     # IO aggiunto per creare il binds per tutti gli elementi
        children.each_with_index{|chi, ii|
           chi.mark_bind
        }
      end
      @seen={}
