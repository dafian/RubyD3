...
require 'rubyvis/scene/svg_group'
...
module Rubyvis
  module SvgScene # :nodoc:
    #include REXML
    def self.svg #not in d3
      "http://www.w3.org/2000/svg" #not in d3
    end #not in d3
    def self.xmlns #not in d3
      "http://www.w3.org/2000/xmlns" #not in d3
    end #not in d3
    def self.xlink #not in d3
       "http://www.w3.org/1999/xlink" #not in d3
    end #not in d3 
    def self.xhtml  #not in d3 
       "http://www.w3.org/1999/xhtml" #not in d3
    end #not in d3
    @scale=1
    def self.scale
      @scale
    ...
    def self.update_all(scenes, tra=nil, i=nil)
      ...
      return self.send(scenes.type, scenes, tra)
    end
    ...
    def self.create(type)
      if Rubyvis.xml_engine==:nokogiri
        ...
        if type=='svg' #not in d3 
        el.add_namespace(nil, self.svg) #not in d3   
        el.add_namespace('xlink', self.xlink) #not in d3   
        end #not in d3 
      else
        el=REXML::Element.new "#{type}"
        if type=='svg' #not in d3
          el.add_namespace(self.svg) #not in d3
          #el.add_namespace("xmlns:xmlns", self.xmlns)
          el.add_namespace("xmlns:xlink", self.xlink) #not in d3
        end #not in d3
      end
      el
    end
    ...
    def self.expect(e, type, attributes, style=nil)
      ...
      attributes.each {|name,value|
        value = nil if (value == IMPLICIT[:svg][name] && name != "x" && name!= "y") # IO aggiunto && name != "x" && name!= "y"
        if (value.nil?)
