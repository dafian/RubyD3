module Rubyvis
  ...
  class Dot < Mark
    ...
    attr_accessor_dsl :shape, :shape_angle, :shape_radius, :shape_size, :line_width, [:stroke_style, lambda {|d| Rubyvis.color(d)}], [:fill_style, lambda {|d| Rubyvis.color(d)}], :classdot, :cx, :cy, :shape_rendering, :fill, :fill_opacity, :stroke, :stroke_opacity, :stroke_width
    ...
    def self.defaults()
      ...
      Dot.new().mark_extend(Mark.defaults).shape("circle"). line_width(1.5).stroke_style("none").cx(nil).cy(nil) # IO modified "lambda {a.scale(self.parent.index" in "none"
    end
