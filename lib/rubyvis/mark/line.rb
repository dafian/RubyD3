module Rubyvis
  ...
  class Line < Mark
    ...
    attr_accessor_dsl :line_width, :stroke_linejoin, [:stroke_style, lambda {|d| Rubyvis.color(d)}], [:fill_style, lambda {|d| Rubyvis.color(d)}], :segmented, :interpolate, :eccentricity, :tension, :classline, :radius, :angle, :shape_rendering, :fill, :fill_opacity, :stroke_opacity, :stroke_width  #IO ho aggiunto la classline
    # Type of line
    def type
      "line"
    end
    ...
    def self.defaults
      ...
      Line.new.mark_extend(Mark.defaults).stroke_linejoin('miter').line_width(1.5).stroke_style("none").interpolate('linear').eccentricity(0).tension(0.7).angle(nil).radius(nil).fill(nil).stroke_opacity(nil)
    end
